...
Workflow.Context.prototype.setBaseUrl=function(url){
  this.baseUrl=url;
}; 
...
Workflow.Context.prototype.loadValue=function(name){
  if(typeof name=="string"){
    if(typeof this.values!="undefined"){
      return Workflow.getProperty(this.values,name);
    }
  }
  return undefined;
};
...
Workflow.Context.prototype.onCompleteTask=function(task,err,result,testResults){
  ...
  if(testResults){
    testResults.results.forEach(function(result){
      if(result.result){
        self.logPass("\t\t","PASS",result.msg);
      } else { 
        self.logFail("\t\t","FAIL",result.msg);
      } 
    });
  }
  this.log("");
