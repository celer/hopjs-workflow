...
/**
  Context -> API calls we can make (global)
  Stack frames -> all variables are global
*/
test = new Workflow.Script("Test the service");
...
test.forEach(["A","B","C","D"],"index","value",function(test){
  test.do("CounterService.create").with({name:"#{value}"});
});
...
test.get("http://localhost:3000/api/counter/#{wolf.name}").outputContains({ name:"wolf"});

test.post("http://localhost:3000/api/counter/").with({ name:"kitten"}).outputContains({ name:"kitten" }).saveOutputAs("kittenHref");
...
test.get("#{kittenHref.href}").errorIsNull().outputContains("Not found");

test.do("TestService.exit");

var context = new Workflow.Context();

//context.setStepper(function(step){
//  setTimeout(step,10);
//});

//What a great place for a generator
