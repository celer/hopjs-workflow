...
Workflow.Tests.outputArrayContains=function(input,output,err,options,context){
  if(err) return Workflow.fail("There was an error");
  if(!(output instanceof Array)) return Workflow.fail("The output was not an array");
  if(context){
    options=JSON.parse(JSON.stringify(options));
    options.value = Workflow.resolve(context,options.value,true);
  }  
  for(var i in output){
    if(Workflow.utils.objectCovers(options.value, output[i])) return Workflow.pass("The output array contained '#{value}'",options);
  }
  return Workflow.fail("The output array did not contain '#{value}'",options);
};

assert.deepEqual(Workflow.Tests.outputArrayContains(null,null,null,{ value: null }),{ result:false, msg:"The output was not an array"});
assert.deepEqual(Workflow.Tests.outputArrayContains(null,[ "hello" ],null,{ value: null }),{ result:false, msg:"The output array did not contain 'null'"});
assert.deepEqual(Workflow.Tests.outputArrayContains(null,[ "wolf" ],null,{ value: "wolf" }),{ result:true, msg:"The output array contained 'wolf'"});
assert.deepEqual(Workflow.Tests.outputArrayContains(null,[ {a:1, b:2 } ],null,{ value: {a:1} }),{ result:true, msg:"The output array contained '{\"a\":1}'"});
...
Workflow.Tests.outputArrayLengthIs=function(input,output,err,options,context){
  if(context){
    options=JSON.parse(JSON.stringify(options));
  ...
  if(p instanceof Array){
    if(typeof options.min!="undefined" && p.length<options.min) return Workflow.fail("The output array length was < #{min}",options);
    if(typeof options.max!="undefined" && p.length>options.max) return Workflow.fail("The output array length was > #{max}",options);
    if(typeof options.min!="undefined" && typeof options.max!="undefined")
      return Workflow.pass("The output array length was between #{min} and #{max}",options);
    if(typeof options.min!="undefined")
      return Workflow.pass("The output array length was > #{min}",options);
    if(typeof options.max!="undefined")
      return Workflow.pass("The output array length was < #{max}",options);
  } return Workflow.fail("The output is not an array",options);
};

assert.deepEqual(Workflow.Tests.outputArrayLengthIs(null,null,null, { property:"foo", value: 1 }),{"result":false,"msg":"The output was null"});
assert.deepEqual(Workflow.Tests.outputArrayLengthIs(null,[1,2,3,4,5], null, { property: "foo", min:1 }),{"result":true,"msg":"The output array length was > 1"});
assert.deepEqual(Workflow.Tests.outputArrayLengthIs(null,[1,2,3,4,5], null, { property: "foo", min:10 }),{"result":false,"msg":"The output array length was < 10"});
assert.deepEqual(Workflow.Tests.outputArrayLengthIs(null,[1,2,3,4,5], null, { property: "foo", max:1 }),{"result":false,"msg":"The output array length was > 1"});
assert.deepEqual(Workflow.Tests.outputArrayLengthIs(null,[1,2,3,4,5], null, { property: "foo", max:10 }),{"result":true,"msg":"The output array length was < 10"});
assert.deepEqual(Workflow.Tests.outputArrayLengthIs(null,[1,2,3,4,5], null, { property: "foo", min:1, max:10 }),{"result":true,"msg":"The output array length was between 1 and 10"});
...
Workflow.Tests.arrayContains=function(input,output,err,options,context){
  ...
  if(output!==null){
    var p = Workflow.getProperty(output,options.property);
    if(p instanceof Array){
...
Workflow.Tests.propertyIsNull=function(input,output,err,options,context){
  if(output){
    var value = Workflow.getProperty(output,options.property);
    if(value===null)
...
Workflow.Tests.propertyChanged=function(input,output,err,options,context){
  if(output){
    if(!input) return Workflow.fail("The input was null");
    var outputValue = Workflow.getProperty(output,options.property);
    var inputValue = Workflow.getProperty(input,options.property);
    if(inputValue!=outputValue) return Workflow.pass("The value changed from '#{inputValue}' to '#{outputValue}'",{ inputValue: inputValue, outputValue: outputValue});
    return Workflow.fail("The value did not change");
  } else return Workflow.fail("The output was null",options);
};

assert.deepEqual(Workflow.Tests.propertyChanged({foo:"hello"},{foo:"hello"},null,{property:"foo"}),{result:false, msg:"The value did not change"});
assert.deepEqual(Workflow.Tests.propertyChanged({foo:"hello"},{foo:"bye"},null,{property:"foo"}),{result:true, msg:"The value changed from 'hello' to 'bye'"});
...
Workflow.Tests.propertyContains=function(input,output,err,options,context){
  if(output){
    ...
    var testValue = Workflow.getProperty(output,options.property);
